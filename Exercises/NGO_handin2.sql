...
CREATE TABLE `Supporter`(
    ...
    email			VARCHAR(30)		NOT NULL	
							CONSTRAINT chk_email CHECK(email REGEXP '^[a-zA-Z0-9._+-]+@[a-zA-Z]+\.[a-zA-Z]+$'),
    phone			CHAR(8) 		NOT NULL 	
							CONSTRAINT chk_phone CHECK(LENGTH(phone) = 8 AND phone REGEXP '^[0-9]+$'),
    city			VARCHAR(30),
    ngo_name		VARCHAR(30),
    birthday		DATE,
    is_volunteer	BOOLEAN			NOT NULL	DEFAULT FALSE,
    FOREIGN KEY (ngo_name) REFERENCES NGO(name)			ON DELETE SET NULL ON UPDATE CASCADE
);
...
-- Write the different sql statement --


-- Write the different sql quiries -- 
